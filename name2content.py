...
from poem_pre import train_word2vec
...
#     train = one_hot[batch]
#     train_y = train[:, 1:]
#     end = np.zeros([train.shape[0], 1], dtype='int')
#     end[end == 0] = -1
#     return np.concatenate([one_hot[end], train_y], axis=1)
def get_label(batch):
    train = one_hot[batch]
    return train
...
model.summary()
model.load_weights("./logs/poem_model_pre/")
...
#                     callbacks=[
#                         ModelCheckpoint("./logs/poem_model_pre/", save_best_only=True, period=1,
